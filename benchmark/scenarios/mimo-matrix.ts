/**
 * 小米 Mimo TTS 矩阵测试场景（向后兼容层）
 *
 * 此文件重定向到新的 matrix 模块，保持 API 兼容性
 */
import type { BenchmarkResult, MatrixFilter, MatrixItem, MimoMatrixConfig } from '../metrics/types';

import {
  MIMO_MATRIX_CONFIG,
  mimoScenarioConfig as matrixScenarioConfig,
  mimoMatrixItems,
  runProviderMatrixScenario,
} from './matrix';

// 重新导出数据
export { matrixScenarioConfig, mimoMatrixItems };

/**
 * 生成矩阵测试的场景标识
 */
export function generateMatrixScenarioName(matrixConfig: MimoMatrixConfig): string {
...
export function printMatrixSummary(): void {
  ...
  console.log('\n=== 小米 Mimo TTS 矩阵测试计划 ===\n');
  console.log('矩阵测试列表:');
  for (const item of mimoMatrixItems) {
 ...
 * 运行矩阵测试场景
 */
export async function runMimoMatrixScenario(options?: {
  iterations?: number;
  filter?: MatrixFilter;
  interval?: number;
  onProgress?: (
    current: number,
    total: number,
    config: MimoMatrixConfig,
    result: BenchmarkResult,
  ) => void;
}): Promise<BenchmarkResult[]> {
  return runProviderMatrixScenario(MIMO_MATRIX_CONFIG, {
    ...options,
    provider: 'mimo',
  });
}
