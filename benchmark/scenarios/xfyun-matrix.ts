...
import {
  xfyunScenarioConfig as matrixScenarioConfig,
...
// 重新导出数据
export { matrixScenarioConfig, xfyunMatrixItems };

/**
 * 生成矩阵测试的场景标识
 */
export function generateMatrixScenarioName(matrixConfig: XfyunMatrixConfig): string {
...
export function calculateMatrixCombinations(): number {
  return xfyunMatrixItems.length;
}
...
export function printMatrixSummary(): void {
  ...
  console.log('\n=== Xfyun TTS 矩阵测试计划 ===\n');
  console.log('矩阵测试列表:');
  for (const item of xfyunMatrixItems) {
 ...
 * 运行矩阵测试场景
 */
export async function runXfyunMatrixScenario(options?: {
  iterations?: number;
  filter?: MatrixFilter;
  interval?: number;
  onProgress?: (
    current: number,
    total: number,
    config: XfyunMatrixConfig,
    result: BenchmarkResult
  ) => void;
}): Promise<BenchmarkResult[]> {
  return runProviderMatrixScenario(XFYUN_MATRIX_CONFIG, {
    ...options,
    provider: 'xfyun',
  });
}
