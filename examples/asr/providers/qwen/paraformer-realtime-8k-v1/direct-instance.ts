/**
 * Paraformer Realtime 8k v1 - 直接实例化示例（Opus 数据包）
...
async function main() {
  ...
  console.log(
    `\n[${timestamp()}] === Paraformer Realtime 8k v1 - 直接实例化 / 流式入/流式出（Opus 数据包）===`
  );
  ...
  try {
    ...
    const asr = new QwenASR({
      ...
      // audioFormat: { sampleRate: 8000 },  // 指定音频采样率（8kHz 电话语音）
    });
