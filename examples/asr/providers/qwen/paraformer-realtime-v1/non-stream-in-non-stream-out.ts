/**
 * Paraformer Realtime v1 - 非流式入/非流式出示例
 * 演示一次性处理完整音频文件的场景
 *
 * 模型特点:
 * - 支持 16kHz 采样率
...
// 固定使用 paraformer-realtime-v1 模型
const MODEL = 'paraformer-realtime-v1';
...
async function main() {
  ...
  console.log(`\n[${timestamp()}] === Paraformer Realtime v1 - 非流式入/非流式出 ===`);
  console.log(`模型: ${MODEL}`);
  console.log(`场景: 文件路径输入 → 完整识别结果输出`);
  console.log(`适用: 标准音频（16kHz 采样率）\n`);
