/**
 * Doubao TTS seed-tts-1.0 - 流式输入/流式输出示例 (ogg_opus 格式)
 * 演示使用 ogg_opus 音频格式的实时语音合成场景
 *
 * 模型特点:
 * - V1 版本
 * - 兼容旧版 API
 ...
 *
 * 音频格式:
 * - ogg_opus: 压缩格式，文件体积小，音质好，适合存储和传输
 *
 * 环境变量:
...
async function main() {
  ...
  console.log(`\n[${timestamp()}] === Seed TTS 1.0 - 流式入/流式出 (ogg_opus) ===`);
  console.log(`模型: ${RESOURCE_ID}`);
  console.log(`格式: ogg_opus`);
  ...
  // 保存音频
  const outputPath = ensureOutputDir(__dirname, basename, 'ogg');
  const buffer = Buffer.concat(chunks.map((c) => Buffer.from(c)));
  writeFileSync(outputPath, buffer);
  console.log(`\n音频已保存至: ${outputPath}`);

  console.log('\n=== 播放提示 ===');
  console.log('OGG Opus 格式播放命令:');
