...
///
/// 初始化双层日志系统：
/// - 文件日志: 写入 `~/.univoice/logs/app.log`（info 级别以上，无 ANSI 颜色）
...
fn get_log_dir() -> PathBuf {
    crate::config::settings::get_home_dir().join(".univoice/logs")
}
...
mod tests {
    ...
    fn test_log_path_uses_project_dir() {
        run_with_temp_home(|_home| {
            let log_path = get_log_path();
            assert!(log_path.starts_with(_home.join(".univoice/logs")));
            assert_eq!(
    ...
    fn test_file_writer_appends() {
        run_with_temp_home(|home| {
            let log_path = home.join(".univoice/logs/app.log");
    ...
    fn test_init_logging_creates_directory() {
        run_with_temp_home(|home| {
            let log_dir = home.join(".univoice/logs");
